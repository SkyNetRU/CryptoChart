...
class ETH_pricesSeeder extends Seeder
{
    /**
    ...
    protected function histoMinute () {
        ...
        for ($i=7;$i >= 0;$i--) {
            ...
            $uri = 'https://min-api.cryptocompare.com/data/histominute?fsym=ETH&tsym=USD&limit=1440&aggregate=1&toTs=' . $toTs;
    ...
    protected function histoHour () {
        ...
        for ($i=0;$i<3;$i++){
            ...
            $uri = 'https://min-api.cryptocompare.com/data/histohour?fsym=ETH&tsym=USD&limit=2000&toTs=' . $toTs;
            $client = new Client(); //GuzzleHttp\Client
            $result = $client->get($uri);
            $prices = json_decode($result->getBody()->getContents());
    ...
    protected function histoDay () {
        ...
        for ($i=0;$i<2;$i++){
            ...
            $uri = 'https://min-api.cryptocompare.com/data/histoday?fsym=ETH&tsym=USD&limit=2000&toTs=' . $toTs;
            $client = new Client(); //GuzzleHttp\Client
            $result = $client->get($uri);
            $prices = json_decode($result->getBody()->getContents());
